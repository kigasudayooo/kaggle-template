...
1. **albumentationsは使用禁止** - Kaggle環境で動作しない
2. **torchvision.transformsを使用** - 唯一の信頼できるオプション
3. **モデル定義はノートブック内に完全定義** - 外部importは不可
4. **weights_only=False** - PyTorch 2.6+でtorch.load()に必須
